import React from "react";

const CardsSvg = (props) => {
  const active = props.active;
  return (
    <svg
      fill={
        active == "cards" ? "var(--primary-foreground)" : "var(--foreground)"
      }
      width="2rem"
      viewBox="0 0 488.201 488.201"
    >
      <g id="SVGRepo_bgCarrier" ></g>
      <g
        id="SVGRepo_tracerCarrier"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M265.2,350.251H33.6c-5.3,0-9.6-4.3-9.6-9.6v-182.6h408.5v24c0,6.6,5.4,12,12,12s12-5.4,12-12v-86.5 c0-18.5-15.1-33.6-33.6-33.6H33.6c-18.6,0-33.6,15.1-33.6,33.6v245.1c0,18.5,15.1,33.6,33.6,33.6h231.7c6.6,0,12-5.4,12-12 S271.9,350.251,265.2,350.251z M33.6,85.951h389.3c5.3,0,9.6,4.3,9.6,9.6v38.5H23.9v-38.5C23.9,90.251,28.3,85.951,33.6,85.951z"></path>{" "}
            <path d="M240.2,247.151c0-6.6-5.4-12-12-12H84c-6.6,0-12,5.4-12,12s5.4,12,12,12h144.2C234.9,259.151,240.2,253.751,240.2,247.151 z"></path>{" "}
            <path d="M84,278.151c-6.6,0-12,5.4-12,12s5.4,12,12,12h57.7c6.6,0,12-5.4,12-12s-5.4-12-12-12H84z"></path>{" "}
            <path d="M82.6,215.651h144.2c6.6,0,12-5.4,12-12s-5.4-12-12-12H82.6c-6.6,0-12,5.4-12,12S75.9,215.651,82.6,215.651z"></path>{" "}
            <path d="M476.3,298.251h-17.6v-28c0-34.7-28.3-63-63-63s-63,28.3-63,63v28h-17.6c-6.6,0-12,5.4-12,12v104c0,6.6,5.4,12,12,12 h161.1c6.6,0,12-5.4,12-12v-104C488.3,303.551,482.9,298.251,476.3,298.251z M356.7,270.151c0-21.5,17.5-39,39-39s39,17.5,39,39 v28h-78V270.151z M464.3,402.151H327.1v-80h137.1v80H464.3z"></path>{" "}
          </g>{" "}
        </g>{" "}
      </g>
    </svg>
  );
};

export default CardsSvg;
